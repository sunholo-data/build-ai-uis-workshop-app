...
| 5.7 | [mcp-app-mutation-round-trip.md](mcp-app-mutation-round-trip.md) | P2 | ~0.5d | v6.1.0 [mcp-app-render-ux.md](../v6.1.0/mcp-app-render-ux.md) ✅, the `window.openai`/postMessage dual-bridge in `infrastructure/mcp-local-demo/` (2026-07-04) | **Workshop/teaching feature.** Demonstrates the MCP Apps **mutation round-trip** (widget-initiated tool call) — the "right way to do mutations": widget emits an action → host calls a server tool (optional approval) → server mutates + returns → result flows to widget AND model. Adds a toy server-side `increment-counter` **data tool** (module-level in-memory state — the server is stateless-per-request) that the `show-demo` widget's **+1** button calls via `window.openai.callTool` (ChatGPT/Copilot) or the SEP-1865 `app.callServerTool` path (Claude/Inspector), then renders the returned count. Fixes a latent bug: `show-demo` sets `openai/widgetAccessible` on itself (meaningless) — moved to the action tool. Mirrored into the global `mcp-app-deploy-test` skill scaffold. Net axiom +6 — SECURE BY CONSTRUCTION (+1, sandboxed widget never gets creds/direct network; deny-by-default widgetAccessible), THIN CLIENT/FAT PROTOCOL (+1), PROTOCOL OVER CUSTOM (+1), GRACEFUL DEGRADATION (+1), OBSERVABLE (+1). No CLI surface (teaching fixture). |
...
| 5.7 | [mcp-app-mutation-round-trip.md](mcp-app-mutation-round-trip.md) | Planned 2026-07-04 (workshop feature; toy `increment-counter` demo of the widget→tool mutation round-trip) |
